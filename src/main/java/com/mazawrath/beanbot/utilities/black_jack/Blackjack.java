package com.mazawrath.beanbot.utilities.black_jack;
...
public class Blackjack {
    ...
    public Blackjack(User[] players, BigDecimal[] bets, ServerTextChannel textChannel) {
    ...
    private void playGame() {
        ...
        for (int i = 0; i < players.length; i++) {
            textChannel.sendMessage("First card: ");
            playerCards.add(new ArrayList<Card>());
            playerTotals[i] += drawCard(newDeck, playerTotals[i], playerCards.get(i));
        }

        textChannel.sendMessage("Second card: ");
        playerTotal += drawCard(newDeck, playerTotal, playerCards);
        System.out.println();
        ...
        while (playerTotal < 21 && another_card) {
            ...
            for (int i = 0; i < playerCards.size(); i++) {
                if (playerCards.get(i).isAce() && playerTotal > 21) {
                    playerTotal -= 10;
                }
            }
        }
     ...
     * @param textChannel allow for user interaction
     * @return the bet on the game
     */
    public static int betAmount(int money, ServerTextChannel textChannel) {
        System.out.println("How much would you like to bet?");
        int bet = Math.abs(textChannel.nextInt());
